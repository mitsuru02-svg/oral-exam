...
- 専門用語には短い言い添えをつける
- 日本語で、Markdownの見出しや箇条書きを使って簡潔に。全体で500〜700字程度`;

const ASK = {
  chart: `この地上天気図について、次の順で説明してください。
1. **気圧配置の型**（西高東低、移動性高気圧、南岸低気圧など）と、その典型的な特徴
2. **高気圧・低気圧・前線の位置**と、進んでいく方向
3. **${FIELD}がその配置のどこに位置するか**
4. **等圧線の混み具合**から読める地上風の強さの傾向（風向も分かれば）
5. 滑空の観点で注目すべき点（上昇風が期待できる配置か、安定か不安定か、風は強いか）`,
...
export async function explainImage(body, env) {
  ...
  if (!out) return json({ error: '解説が空でした' }, 502);

  return json({ text: out, model: MODEL, url }, 200);
}
