...
- 専門用語には短い言い添えをつける
- **時刻はすべて日本時間（JST）で書く**。METAR/TAFの電文に出てくる時刻（090500Zや
  FM1006、BECMG 1006/1008、TEMPO 1010/1013など）は世界時（UTC）なので、
  9時間足してJSTに直してから書く（日付をまたぐ場合は日付も進める）。
  「06Z」「1006Z」のようにUTCのまま書かない
- **雲の量や広がりは、実際に見えている程度をそのまま言う**。一部の時間・一部の場所にしか見えないものを
  「全体的に」「一面に」のように広げて言わない。逆に、複数コマのうち一部で見えているものを
  無かったことにもしない。「終始少ない」「一部の時間帯だけ薄雲がかかる」「終始広く覆う」を
  はっきり区別する。グライダーにとって雲は重要な判断材料なので、この区別は特に丁寧に扱う
...
3. **${FIELD}周辺のいま**の雲の状況。基本的に少ないのか、終始広く覆っているのか、
   一部のコマ・時間帯だけ薄雲がかかっているのかを区別して言う
...
2. **雲の実態**（②③）—— 雲域がどちらへ動いているか、発達か衰えか、積雲は立っているか。
   ${FIELD}周辺が終始少ないのか、終始広く覆うのか、一部の時間帯だけ薄雲がかかるのかを区別する
...
7. **試験官への説明の組み立て** —— 上の内容を口頭で説明するとしたら、
   どの順で、どの数字を挙げて話すか。**雲の状態には必ず触れる**こと
   （量・高さ・種類に加えて、終始少ないのか、終始広く覆うのか、
   一部の時間帯だけ薄雲がかかるのかを、②③の実態に沿って言う。
   グライダーにとって雲は重要な判断材料なので、他の要素と同格以上に扱う）。
   3〜4文の例を示す
...
3. **${FIELD}周辺のいま**の雲の状況（雲の厚み、粒の細かさ）。基本的に少ないのか、
   終始広く覆っているのか、一部のコマ・時間帯だけ筋状の薄雲が見えるのかを区別して言う
4. **上昇風（サーマル）が期待できそうか**。積雲列が並んでいれば、その向き
5. 滑空の観点で注目すべき点`,

  metar: `渡したのは近傍空港（旭川RJEC・丘珠RJCO・新千歳RJCC）のMETAR/TAFの
電文です。画像ではなく文字の電文なので、そのつもりで読んでください。
${FIELD}そのものの観測ではない（いちばん近いのは旭川ですが、それでも
別地点）ため、あくまで参考としてどう解釈するかを含めて述べるのが要点です。

1. **各空港の現在の実況**（風向風速・視程・雲底）と、3空港を見比べたときの
   傾向・違い
2. **TAFから読み取れる、これからの時間帯ごとの変化**（FM・BECMG・TEMPOが
   いつ、何を示しているか）
3. **${FIELD}にとってどう読めるか**。旭川が地理的に近いことを踏まえつつ、
   3空港の傾向から滝川周辺の風・視程・雲底の見通しを述べる
4. 飛行に関わる変化のタイミング（風向風速・視程・雲底が変わりそうな時間帯）`,
};
...
/* 見出しや前置き、字数の断り書き、資料の出典などを付けてくることが
   あったため、末尾に明示で禁じる一文を添えてある */
const CONCISE_TAIL = '\n見出しや前置き、資料の出典、字数の断り書き（「約120字」等）は'
  + '書かず、本文の文章だけを渡すこと。';
...
飛べる・飛べないの判断はしないこと。${CONCISE_TAIL}`,

  sigwx: `渡した下層悪天予想図（3・6・9時間後）から、要点だけを簡潔に。
${FIELD}周辺の天気・風（矢羽根から読める風向風速）の状況と、
時間とともにどう変わるかを、120字程度の短い文章でまとめてください。
箇条書きにせず、飛べる・飛べないの判断はしないこと。${CONCISE_TAIL}`,

  ir: `渡した赤外衛星画像（古い順）から、要点だけを簡潔に。
${FIELD}周辺の天気（雲の状況。終始少ないのか、広く覆うのか、一部の時間帯だけ
薄雲がかかるのかを区別すること）と、雲の動きから読める風向の傾向、
このあとの見通しを、120字程度の短い文章でまとめてください。
箇条書きにせず、飛べる・飛べないの判断はしないこと。${CONCISE_TAIL}`,

  vis: `渡した可視衛星画像（古い順）から、要点だけを簡潔に。
${FIELD}周辺の天気（雲の状況・積雲の有無。終始少ないのか、広く覆うのか、
一部の時間帯だけ筋状の薄雲が見えるのかを区別すること）と、雲の動きから読める
風向の傾向、このあとの見通しを、120字程度の短い文章でまとめてください。
箇条書きにせず、飛べる・飛べないの判断はしないこと。${CONCISE_TAIL}`,

  metar: `渡したのは近傍空港（旭川・丘珠・新千歳）のMETAR/TAFの電文です。
要点だけを簡潔に。${FIELD}そのものの観測ではない点も踏まえつつ、
周辺の天気・風向風速の傾向と、これからの見通しを、120字程度の
短い文章でまとめてください。箇条書きにせず、
飛べる・飛べないの判断はしないこと。${CONCISE_TAIL}`,
};
...
export async function explainImage(body, env) {
  ...
  /* ⑤（近傍空港の電文）は画像を伴わず、文字だけで解説させる。
     images が空でも kind:'metar' なら弾かず、電文の有無で判定する */
  const extra = String((body && body.text) || '').slice(0, 6000).trim();
  const isTextOnly = kind === 'metar';
  if (isTextOnly && !extra) {
    return json({ error: '電文がありません' }, 400);
  }

  /* 1枚でも複数枚でも受ける。複数のときは images:[{label,url}]。
     衛星画像は時系列で読ませたいので、10枚ぶんを見込んで上限を取る */
  const list = isTextOnly ? [] : (Array.isArray(body && body.images) && body.images.length
    ? body.images.slice(0, kind === 'brief' ? 10 : 12)
    : [{ label: '', url: String((body && body.url) || '') }]);

  if (!isTextOnly && !list.every(it => IMG_OK.test(String(it && it.url)))) {
  ...
  try {
    r = await fetch(ANTHROPIC_URL, {
      ...
      body: JSON.stringify({
        model: MODEL,
        /* 上限は「途中で切れて注意書きが付く」のを避けるため、
           狙う字数よりだいぶ余裕を持たせてある。上限に届かなければ
           費用は実際に書いた分だけなので、余裕を持たせても損はない。
           それでも brief（①〜⑤まとめ、7項目）で実際に max_tokens に
           当たって途中で切れることがあったため、さらに余裕を増やした */
        max_tokens: concise ? 2000 : (kind === 'brief' ? 8000 : 6000),
        system: SYSTEM,
